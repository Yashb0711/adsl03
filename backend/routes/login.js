...
router.post("/user", (req, res) => {
    ...
    try {
        const databaseName = "USERS", username = req.body.username, password = req.body.password;
        const query = "SELECT * FROM " + databaseName + " WHERE user_id=?";
        connection.query(query, [username], (err, result) => {
            if (err) {
                res.status(400).send(err.message);
                return;
                ...
                if (result.length == 0)
                    ...
                    if (result[0].password === password.trim())
                        if (result[0].userType === 'Student') {
                            res.status(200).send({ loggedIn: true, userType: 'Student' });

                        }
                        else {
                            res.status(200).send({ loggedIn: true, userType: "Faculty" });
                            console.log("Helllow")
                        }
                    else
                        res.status(400).send({ message: "Username and password does not match" });
